...
async function isPageBlank(
  ...
  const app = (extensionPath: string) => {
    console.log("Extension path part 1 ---> ", extensionPath);
  ...
  try {
    ...
    if (appElementCount === 0) {
      console.log(
        `[WaitForExtensionOnLoadPage] ${app(
          extensionPath
        )} app element not found, page appears to be blank`
      );
      return true;
    }
    ...
    console.warn(
      `[WaitForExtensionOnLoadPage] Error checking if page is blank: ${
...
async function fixBlankPage(
  ...
  for (let attempt = 0; attempt < MAX_BLANK_PAGE_RETRIES; attempt++) {
    console.log(
      `[WaitForExtensionOnLoadPage] Checking page state (attempt ${
        attempt + 1
      }/${MAX_BLANK_PAGE_RETRIES})`
    );
    ...
    if (isBlank) {
      ...
      console.log(
        "[WaitForExtensionOnLoadPage] Page appears to be loaded correctly"
      );
      return true;
    }
  }
  ...
  console.warn(
    "[WaitForExtensionOnLoadPage] Failed to fix blank/error page after multiple attempts"
  );
  return false;
}
 ...
 */
async function findExtensionPage(
...
export async function waitForExtensionOnLoadPage(
  ...
  // Initial delay to ensure browser is fully initialized
  console.log(
    `[WaitForExtensionOnLoadPage] Waiting ${INITIAL_BROWSER_DELAY}ms for browser to initialize...`
  );
  ...
  while (retries <= MAX_RETRIES) {
    try {
      console.log(
        `[WaitForExtensionOnLoadPage] Looking for extension page (attempt ${
          retries + 1
        }/${MAX_RETRIES + 1})...`
      );
      ...
      const maxPollingAttempts = Math.floor(
        EXTENSION_LOAD_TIMEOUT / POLLING_INTERVAL
      );
      ...
      while (pollingAttempts < maxPollingAttempts) {
        ...
        if (extensionPage) {
          console.log(
            `[WaitForExtensionOnLoadPage] Found extension page after ${
              pollingAttempts + 1
            } polling attempts`
          );
          break;
      ...
      console.log(
        "[WaitForExtensionOnLoadPage] Checking if extension page is properly loaded..."
      );
      const isFixed = await fixBlankPage(extensionPage, extensionPath);

      if (isFixed) {
        console.log(
          "[WaitForExtensionOnLoadPage] Extension page is properly loaded and ready"
        );
      ...
      const pages = context.pages();
      console.log(
        `[WaitForExtensionOnLoadPage] Current browser has ${pages.length} pages open`
      );
      for (let i = 0; i < pages.length; i++) {
        ...
        if (page) {
          try {
            console.log(
              `[WaitForExtensionOnLoadPage] Page ${i + 1}: ${page.url()}`
            );
          } catch (urlError) {
            console.log(
              `[WaitForExtensionOnLoadPage] Page ${i + 1}: Unable to get URL`
            );
          }
        }
      }
  ...
  throw new Error(
    "[WaitForExtensionOnLoadPage] Unexpected end of function reached"
  );
}
