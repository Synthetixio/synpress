...
const isRunningInWsl = (): boolean => {
  try {
    const releaseContent = fs
      .readFileSync("/proc/version", "utf8")
      .toLowerCase();
    return (
      releaseContent.includes("microsoft") || releaseContent.includes("wsl")
    );
  } catch (error) {
    return false;
  }
...
export const cliEntrypoint = async () => {
  console.log(
    `⚠️ ${chalk.yellowBright`The CLI is in alpha so expect breaking changes!`} ⚠️\n`
  );
  ...
  const program = new Command()
    ...
    .option(
      "-f, --force",
      "Force the creation of cache even if it already exists",
      false
    )
    .option(
      "-d, --debug",
      "If this flag is present, the compilation files are not going to be deleted",
      false
    )
    .option(
      "-p, --phantom",
      "If this flag is present, Phantom extension will be installed instead of Metamask",
      false
    )
    .option(
      "-ptr, --petra",
      "If this flag is present, Petra extension will be installed instead of Metamask",
      false
    )
  ...
  console.log(
    chalk.greenBright(
      "🚀 Building the cache for wallet setup functions... 🚀\n"
    )
  );

  const { outDir: compiledWalletSetupDirPath, setupFunctionHashes } =
    await compileWalletSetupFunctions(walletSetupDir, flags.debug);

  // TODO: We should be using `prepareExtension` functions from the wallet itself!
  if (flags.phantom) {
    await createCache(
      compiledWalletSetupDirPath,
      setupFunctionHashes,
      prepareExtensionPhantom,
      flags.force
    );
  } else if (flags.petra) {
    await createCache(
