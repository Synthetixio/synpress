...
describe("createCache", () => {
  ...
  it("calls getUniqueWalletSetupFunctions with correct arguments", async () => {
    const getUniqueWalletSetupFunctionsSpy = vi.spyOn(
      GetUniqueWalletSetupFunctions,
      "getUniqueWalletSetupFunctions"
    );
  ...
  it("calls triggerCacheCreation with correct arguments", async () => {
    const triggerCacheCreationSpy = vi.spyOn(
      TriggerCacheCreation,
      "triggerCacheCreation"
    );

    const downloadExtension = vi.fn(async () =>
      path.join(ROOT_DIR, "extension")
    );
    await createCache(ROOT_DIR, functionStrings, downloadExtension, false);

    expect(triggerCacheCreationSpy).toHaveBeenCalledOnce();
    expect(triggerCacheCreationSpy).toHaveBeenCalledWith(
      setupFunctions,
      functionStrings,
      downloadExtension,
      false
  ...
  it("does nothing if no setup functions need caching", async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      "No new setup functions to cache. Exiting..."
    );
  });
