...
test("should add network and close network added popup", async ({
  metamaskPage,
  createAnvilNode,
  metamask,
}) => {
  ...
  const isNetworkAddedPopupVisible = await waitFor(
    () =>
      metamaskPage
        .locator(
          metamask.homePage.selectors.networkAddedPopover.switchToNetworkButton
        )
        .isVisible(),
    3_000,
    false
  ...
  await expect(
    metamaskPage.locator(metamask.homePage.selectors.currentNetwork)
  ).toHaveText(network.name);
});

test("should add network without block explorer", async ({
  metamaskPage,
  createAnvilNode,
  metamask,
}) => {
...
test("should throw if there is an issue with chain id", async ({
  metamask,
  createAnvilNode,
}) => {
  const { rpcUrl, chainId } = await createAnvilNode();
