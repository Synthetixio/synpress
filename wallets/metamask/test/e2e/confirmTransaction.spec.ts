...
const test = testSynpress.extend<{
  ...
  connectAndTriggerEIP1559Transaction: async (
    { page, connectToAnvil },
    use
  ) => {
    await use(async () => {
...
describe("with default gas setting", () => {
  test("should confirm contract deployment", async ({
    page,
    metamask,
    connectToAnvil,
  }) => {
  ...
  test("should confirm legacy transaction", async ({
    page,
    metamask,
    connectToAnvil,
  }) => {
  ...
  test("should confirm EIP-1559 transaction", async ({
    metamask,
  ...
  describe("NFTs", () => {
    test("should confirm `watch NFT` request", async ({
      page,
      metamask,
      connectDeployAndMintNft,
    }) => {
    ...
    test("should confirm `watch all NFTs` request", async ({
      page,
      metamask,
      connectDeployAndMintNft,
    }) => {
    ...
    test("should confirm `approve` transaction", async ({
      page,
      metamask,
      connectDeployAndMintNft,
    }) => {
    ...
    test("should confirm `set approval for all` transaction", async ({
      page,
      metamask,
      connectDeployAndMintNft,
    }) => {
      ...
      await expect(page.locator("#nftsStatus")).toHaveText(
        "Set Approval For All completed"
      );
    });

    test("should confirm `revoke` transaction", async ({
      page,
      metamask,
      connectDeployAndMintNft,
    }) => {
    ...
    test("should confirm `transfer from` transaction", async ({
      page,
      metamask,
      connectDeployAndMintNft,
    }) => {
...
describe("with custom gas setting", () => {
  test('should throw due to no estimation for "low" gas setting', async ({
    ...
    await expect(
      metamask.confirmTransaction({ gasSetting: "low" })
  ...
  test('should throw due to no estimation for "market" gas setting', async ({
    ...
    await expect(
      metamask.confirmTransaction({ gasSetting: "market" })
    ).rejects.toThrowError(
  ...
  test('should throw due to no estimation for "aggressive" gas setting', async ({
    ...
    await expect(
      metamask.confirmTransaction({ gasSetting: "aggressive" })
    ).rejects.toThrowError(
  ...
  describe("with advanced (manual) gas setting", () => {
    ...
    test("should throw if `gasLimit` is too low", async ({
      metamask,
      connectAndTriggerEIP1559Transaction,
    }) => {
      await connectAndTriggerEIP1559Transaction();
    ...
    test("should confirm transaction with small gas fee", async ({
      metamask,
      connectAndTriggerEIP1559Transaction,
    }) => {
      await connectAndTriggerEIP1559Transaction();
    ...
    test("should confirm transaction with huge gas fee", async ({
      metamask,
      connectAndTriggerEIP1559Transaction,
    }) => {
      await connectAndTriggerEIP1559Transaction();
...
describe("with `from` and `to` specified", () => {
  test("should confirm from/to transfer", async ({
    page,
    metamask,
    deployToken,
  }) => {
    ...
    await page
      .locator("#transferFromRecipientInput")
...
describe("using custom transaction form", () => {
  test("should send defined amount", async ({
    page,
    metamask,
    connectToAnvil,
  }) => {
