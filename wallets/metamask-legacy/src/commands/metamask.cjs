const log = require('debug')('synpress-legacy:metamask')
...
} = require('../pages/metamask/first-time-flow-page.cjs')
const { mainPageElements } = require('../pages/metamask/main-page.cjs')
const { unlockPageElements } = require('../pages/metamask/unlock-page.cjs')
const {
...
} = require('../pages/metamask/notification-page.cjs')
const {
...
} = require('../pages/metamask/settings-page.cjs')
const {
  confirmationPageElements
} = require('../pages/metamask/confirmation-page.cjs')
const {
  setNetwork,
  addNetwork,
  findNetwork,
...
const metamask = {
  ...
  async closeModal() {
    ...
    if (
      ...
      await playwright.waitAndClick(mainPageElements.connectedSites.closeButton)
    }
    return true
  },
  ...
  async importWallet(secretWords, password) {
    ...
    await playwright.waitAndClick(firstTimeFlowImportPageElements.termsCheckbox)
    await playwright.waitAndClick(
  ...
  async createAccount(accountName) {
    ...
    if (accountExists) {
      ...
      await playwright.waitAndClick(mainPageElements.createAccount.cancelButton)
    } else {
