...
test("should confirm `personal_sign`", async ({ page, petra }) => {
...
test("should confirm `eth_signTypedData`", async ({ page, petra }) => {
  await connectPetraToTestDapp(page, petra);
  ...
  await expect(page.locator("#signTypedDataVerifyResult")).toHaveText(
    "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266"
  );
});

test("should confirm `eth_signTypedData_v3`", async ({ page, petra }) => {
  await connectPetraToTestDapp(page, petra);
  ...
  await expect(page.locator("#signTypedDataV3VerifyResult")).toHaveText(
    "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266"
  );
});

test("should confirm `eth_signTypedData_v4`", async ({ page, petra }) => {
  await connectPetraToTestDapp(page, petra);
  ...
  await expect(page.locator("#signTypedDataV4VerifyResult")).toHaveText(
    "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266"
  );
});

test("should confirm `eth_sign`", async ({ page, petra }) => {
  await connectPetraToTestDapp(page, petra);

  await page.locator("#ethSign").click();

  await petra.confirmSignatureWithRisk();
